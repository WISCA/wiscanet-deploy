function h = lseChanEstimate( s, z, lags )
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here
%   (S'*S)\(S'*z); (stored along columns)
...
function YLag = lagMtx(Y,lags)
%LAGMTX Create matrix of lagged time series
%
% Syntax:
%
%   YLag = lagMtx(Y,lags)
...
% Copyright 1999-2010 The MathWorks, Inc.   
    ...
    if nargin ~= 2
        ...
        error(message('lagMtx:UnspecifiedInput'))
    ...
    if numel(lags) ~= length(lags) % Check for vector
       ...
       error(message('lagMtx:NonVectorLags'))
    ...
    if any(round(lags)-lags)
       ...
       error(message('lagMtx:NonIntegerLags'))

    end

    missingValue = 0;  % Assign default missing value
